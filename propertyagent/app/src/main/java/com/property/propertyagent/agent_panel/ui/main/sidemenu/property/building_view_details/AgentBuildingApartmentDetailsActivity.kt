...
class AgentBuildingApartmentDetailsActivity : BaseActivity() {
    ...
    override val layoutId: Int
        get() = R.layout.activity_agent_building_apartment_details
    ...
    override fun initData() {
        ...
        buildingDetailsViewModel.fetchAgentApartmentDetails(passedPropertyId, clicked_building_id)
    }
    ...
    override fun setupObserver() {
        buildingDetailsViewModel.getAgentApartmentDetailsResponse().observe(this, {
            when (it.status) {
                Status.SUCCESS -> {
                    dismissProgress()
                    if (it.data?.response != null) {
                        ...
                        if (!it.data.response.equals(null)) {
                            if (!it.data.response.property.equals(null)) {
                                ...
                                AppPreferences.property_type = it.data.response.property.property_to
        ...
        buildingDetailsViewModel.getAgentPropertyPdfResponse().observe(this, {
            when (it.status) {
                Status.SUCCESS -> {
                    dismissProgress()
    ...
    override fun onClicks() {
        ...
        btnDownload.setOnClickListener {
            buildingDetailsViewModel.agentPropertyPdf(passedPropertyId)
        }
    }

    override fun onResume() {
        super.onResume()
        if (AppPreferences.isConnectionRestored) {
            AppPreferences.isConnectionRestored = false
            if (passedPropertyId.isNotBlank()) {
                buildingDetailsViewModel.fetchAgentApartmentDetails(
                    passedPropertyId,
                    clicked_property_id
                )
            }
