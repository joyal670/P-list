...
import com.property.propertyuser.modal.filter_home.TypeModel
import kotlinx.android.synthetic.main.list_home_filter_main_item.view.*


class HomeSeletedFilteredMainCategoryItemAdapter_1  (private val context: Context,
                                                     private var  selectedFilterMainItems: List<TypeModel>,
                                                     private val  functionFilterMainMain:(String)->Unit)
    : RecyclerView.Adapter<HomeSeletedFilteredMainCategoryItemAdapter_1.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ViewHolder,
                                  position: Int) {
        holder.itemView.tvFilterItemMain.text=selectedFilterMainItems[position].typeModelName
        holder.itemView.tvFilterItemMain.setOnClickListener {
            rowIndex=position
            notifyDataSetChanged()
            functionFilterMainMain.invoke(selectedFilterMainItems[position].typeModelName)
        }
