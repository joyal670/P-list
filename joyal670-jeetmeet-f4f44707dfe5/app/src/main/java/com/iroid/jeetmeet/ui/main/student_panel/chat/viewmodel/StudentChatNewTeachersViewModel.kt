...
class StudentChatNewTeachersViewModel : ViewModel() {
    /* variables */
    private val studentTeacherLiveData = MutableLiveData<Resource<StudentChatNewTeacherResponse>>()

    val studentTeacherData: LiveData<Resource<StudentChatNewTeacherResponse>>
        get() = studentTeacherLiveData

    /* Teacher list */
    fun studentChatTeacher(teacher_id: Int) {
        val repository = ApiRepositoryProvider.providerApiRepository()
        viewModelScope.launch {
            try {
                studentTeacherLiveData.postValue(Resource.loading(null))
                repository.studentChatNewTeacher(teacher_id).let {
